...
def reorder_invalid_list(list_int, orderings):
    ...
    for i in range(length):
        ...
        for j in range((i+1),length):
            ...
            if items == None:
                ...
                #print(f"DEBUG: Higher priority: current = {current} , next = {next}")
                if next not in new_list:
...
def solve_part2(filename):
    ...
    for l in lines:
        ...
        for v in values:
            ...
            list_int.append(entry)
        #print(f"DEBUG: list_int={list_int}")   
        if not is_valid_ordering(list_int, orderings):
            print(f"DEBUG: Invalid list_int={list_int}")   

            #TODO: Reoder list
            new_list = list_int
            while not is_valid_ordering(new_list, orderings):
                new_list = reorder_invalid_list(new_list, orderings)
            #new_list = reorder_invalid_list(new_list, orderings)    
