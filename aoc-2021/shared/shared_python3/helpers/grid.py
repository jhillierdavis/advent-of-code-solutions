...
def grid_to_lines(grid):
    ...
    for h in range(grid.getHeight()):
        ...
        for w in range(grid.getWidth()):
            line += grid.getSymbol(point.Point2D(w,h))
        lines.append(line)
    return lines
...
class Grid2D():
    ...
    def getSymbol(self, coord:point.Point2D) -> chr:
        if not self.contains(coord):
            raise(Exception("Point {coord} not within grid!"))
        return self.array[coord.getX()][coord.getY()]

    def setSymbol(self, coord:point.Point2D, symbol:chr):
        if not self.contains(coord):
            raise(Exception("Point {coord} not within grid!"))
    ...
    def getCardinalPointNeighbours(self, p:point.Point2D) -> set:
        neightbours = set()
    ...
    def getSurroundingNeighbours(self, p:point.Point2D) -> set:
        neightbours = set()
